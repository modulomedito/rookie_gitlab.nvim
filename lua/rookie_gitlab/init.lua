...
local function create_ui_buffer()
    if state.buf and vim.api.nvim_buf_is_valid(state.buf) then
        ...
        keymaps.setup_ui_buffer(state.buf)
    end
...
function M.add_issue(edit_iid)
    ...
    if is_edit then
        local issue = make_request(string.format("/projects/%d/issues/%d", project_id, edit_iid))
        if issue then
    ...
    local function close_action()
        ...
        if submitted_data and submitted_data.title ~= "" then
            ...
            if res then
                ...
                vim.notify("[RkGitlab] Failed to " .. action_str .. " issue", vim.log.levels.ERROR)
            end
        else
    ...
    local ctx = {
        ...
        buf_desc = buf_desc,
        buf_flags = buf_flags,
        buf_confirm = buf_confirm,
        buf_cancel = buf_cancel,
        win_title = win_title,
        win_desc = win_desc,
        win_flags = win_flags,
        win_confirm = win_confirm,
